...
class External_moderator extends CI_Controller
{
	public function __construct()
	{
		parent::__construct();
		if (empty($_SESSION['external_moderator']['id'])) {
			redirect('external-moderator');
		} else {
			$assessor = $this->common->accessrecord('external_moderator', [], ['id' => $this->session->userdata['external_moderator']['id']], 'row');
	...
	{
		$this->data['external_moderation_report_'] = $this->common->accessrecord('external_moderation_report', [], ['moderator_id' => $this->session->userdata['external_moderator']['id']], 'result');
		$this->data['external_moderation_report'] = count($this->data['external_moderation_report_']);

		$this->data['page'] = 'dashboard';

		$this->data['content'] = 'pages/dashboard/dashboard';

		$this->load->view('external_moderator/tamplate', $this->data);
	}
	...
	public function logout()
	{
		...
		$this->session->unset_userdata("external_moderator");
	...
	public function moderator_changepassword()
	{
		...
		if ($_POST) {
			...
			if (isset($_SESSION['external_moderator']['id'])) {

				$id = $_SESSION['external_moderator']['id'];
			} else {
			...
			$datau = $this->common->accessrecord('external_moderator', ['id, password'], array('id' => $id), 'row');

			if (!empty($datau)) {

				if ($datau->password == sha1($oldpassword)) {

					$this->common->updateData('external_moderator', array('password' => sha1($password)), array('id' => $id));

					$this->session->set_flashdata('success', 'Your Password Successfully Update');

					redirect('external-moderator-changepassword');
				} else {
	...
	public function moderator_editprofile()
	{
		...
		if (isset($_SESSION['external_moderator']['id'])) {
			...
			$moderator_id = $_SESSION['external_moderator']['id'];
		} else {
		...
		if ($_POST) {
			...
			if (!empty($_FILES['profile_image']['name'])) {
				...
				$path = './uploads/external_moderator/profile_image/';
				...
				$moderator = $this->common->accessrecord('external_moderator', [], ['id' => $moderator_id], 'row');

				$profile_image = $moderator->profile_image;
			}
			...
			if ($this->common->updateData('external_moderator', $data, ['id' => $moderator_id])) {

				$this->session->set_flashdata('success', 'Profile Updated Successfully');

				redirect('external-moderator-editprofile');
			} else {
			...
			$this->data['record'] = $this->common->accessrecord('external_moderator', [], ['id' => $moderator_id], 'row');
	...
	public function create_moderation()
	{
		...
		if (empty($_POST)) {
			...
			$this->data['moderator_Record'] = $this->common->accessrecord('external_moderator', [], ['id' => $moderator_id], 'row');
			...
			$this->data['page'] = 'createmoderationexe';
			...
			$res = $this->common->insertData('external_moderation_report', $data);
			...
			foreach ($bulk as $key => $value) {
				...
				$respn = $this->common->insertData('external_moderation_report_learner', $datal);
			}
			...
			if ($respn) {
				...
				redirect('external-moderator-moderation-list');
			} else {

				$this->session->set_flashdata('error', 'Please Try Again');
				redirect('external-moderator-create-moderation');
			}
		}
	}
...
// 	}
	...
	{
		if (isset($_SESSION['external_moderator']['id'])) {
			$moderatorid = $_SESSION['external_moderator']['id'];
			$moderator = $this->common->accessrecord('external_moderator', [], ['id' => $moderatorid], 'row');
	...
	public function moderationlist()
	{
		...
		$this->data['moderation_report'] = $this->common->accessrecord('external_moderation_report', [], ['moderator_id' => $moderator_id], 'result_array');
	...
	public function user_listing()
	{
		...
		if (!empty($_GET['id'])) {
			...
			$id = $_GET['id'];
			$this->data['moderation_record'] = $this->common->accessrecord('external_moderation_report', [], ['id' => $id], 'row');
			$this->data['learner_record'] = $this->common->accessrecord('external_moderation_report_learner', [], ['moderation_report_id' => $id], 'result_array');
		}
