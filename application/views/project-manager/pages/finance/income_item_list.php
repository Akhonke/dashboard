            ...
            <div class="col-lg-12 mb-1">
                <div class="card">
                    <div class="card-header">
                        <h3 class="h6 text-uppercase mb-0">Income List</h3>
                    </div>
                    <div class="card-body">
                                            ...
                                            <tr id="del-<?= $value->id ?>">
                                                ...
                                                <td>
                                                    ...
                                                    <?php if ($res['is_edit'] == 1) { ?>
                                                        <a href="projectmanager-create-income-item?id=<?= $value->id ?>" class="btn btn-info btn-sm"><i class="fa fa-edit"></i></a>

                                                    <?php }
                                                    if ($res['is_delete'] == 1) { ?>
                                                        <a href="javascript:;" class="btn btn-danger btn-sm" style="margin:5px 0 0" onclick="deleterecord(' finance_income_item&behalf','id',<?= $value->id ?>)"><i class="fa fa-trash"></i></a>
                                                    <?php } ?>
                                                </td>
...
<script>
    function view() {
        ...
        $("#exampleModal").modal({
            ...
            var date = $(event.relatedTarget).closest("tr").find("td:eq(4)").text();
            var amount = $(event.relatedTarget).closest("tr").find("td:eq(5)").text();
            var payer = $(event.relatedTarget).closest("tr").find("td:eq(6)").text();
            var description = $(event.relatedTarget).closest("tr").find("td:eq(7)").text();
            ...
            $(this).find("#personDetails").html($("<div class=row><div class=col-sm-12><span class='pull-left'>Project Manager :</span><span class='pull-right'>  " + project_nm +
                ...
                "</div><div class=col-sm-12><span class='pull-left'>Payer :</span><span class='pull-right'> " + payer +
                "</div><div class=col-sm-12><span class='pull-left'>Description: </span><span class='pull-right'> " + description +
