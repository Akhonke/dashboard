<div class="col-md-12">
   <nav aria-label="breadcrumb">
      <ol class="breadcrumb pl-0">
         ...
         <li class="breadcrumb-item active" aria-current="page">Paid And Free Users List</li>
      </ol>
   </nav>
   <div class="ms-panel">
      <div class="ms-panel-header ms-panel-custome">
         <h6>Paid And Free Users List</h6>
      </div>
    ...
    function view(){
        ...
        debugger;
  		...
  		$("#exampleModal").modal({
            ...
            var paid_and_free = $(event.relatedTarget).closest("tr").find("td:eq(1)").text(); 
  			var fullname = $(event.relatedTarget).closest("tr").find("td:eq(2)").text(); 
            var email_address = $(event.relatedTarget).closest("tr").find("td:eq(3)").text(); 
            var mobile_number = $(event.relatedTarget).closest("tr").find("td:eq(4)").text(); 
  			var packageName = $(event.relatedTarget).closest("tr").find("td:eq(5)").text(); 
            var packageCreatedDate = $(event.relatedTarget).closest("tr").find("td:eq(6)").text(); 
            var packageExpiryDate = $(event.relatedTarget).closest("tr").find("td:eq(7)").text(); 
            var packagePrice = $(event.relatedTarget).closest("tr").find("td:eq(8)").text(); 
           
  			//displays values to modal
              $(this).find("#personDetails").html($("<div class=row><div class=col-sm-12><span class='pull-left'>Paid And Free :</span><span class='pull-right'> " + paid_and_free + 
              "</div><div class=col-sm-12><span class='pull-left'>Name : </span><span class='pull-right'>" + fullname + 
